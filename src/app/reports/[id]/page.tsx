...
function EditForm() {
    ...
    getReport(reportId).then(r => {
      setForm({
        ...
        sections: (r.report_sections ?? []).map((s: any) => ({
          ...
          photos: [],
          existingPhotoIds: (r.photos ?? []).filter((p: any) => p.section_id === s.id).map((p: any) => p.id),
        })),
        gallery_photos: [],
      })
  ...
  function upd(k: keyof ReportFormData, v: any) { setForm(f => f ? { ...f, [k]: v } : f) }
  function updSec(i: number, p: Partial<SectionFormData>) {
    setForm(f => { if (!f) return f; const s = [...f.sections]; s[i] = { ...s[i], ...p }; return { ...f, sections: s } })
  }
  ...
  if (loading) return <div style={{ display: 'flex', justifyContent: 'center', padding: 48 }}><div style={{ width: 22, height: 22, border: '2px solid #d8d4cc', borderTopColor: '#1a1916', borderRadius: '50%', animation: 'spin 0.8s linear infinite' }} /><style>{`@keyframes spin{to{transform:rotate(360deg)}}`}</style></div>
  if (!form) return <div style={{ textAlign: 'center', padding: 48, color: '#9c9890' }}>読み込めませんでした</div>
  ...
  return (
    <div style={{ maxWidth: 640, margin: '0 auto' }}>
      ...
      <div style={{ padding: 16, display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          {form.sections.map((sec, si) => (
            <div key={si} style={{ background: '#fff', borderRadius: 14, border: '0.5px solid #d8d4cc', overflow: 'hidden', marginBottom: 10 }}>
              ...
              <div style={{ padding: 14 }}>
                ...
                {sec.mode === 'text' ? (
                  <textarea placeholder="作業内容を記入..." value={sec.content} onChange={e => updSec(si, { content: e.target.value })} rows={3}
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                    {sec.items.map((item, ii) => (
                      <div key={ii} style={{ display: 'flex', alignItems: 'center', gap: 8, background: '#f5f4f0', borderRadius: 8, padding: '7px 10px' }}>
                        <input type="checkbox" checked={item.done} onChange={e => { const items = sec.items.map((it, j) => j === ii ? { ...it, done: e.target.checked } : it); updSec(si, { items }) }}
                          style={{ width: 15, height: 15, flexShrink: 0, accentColor: '#27735a' }} />
                        <input placeholder="項目名..." value={item.text} onChange={e => { const items = sec.items.map((it, j) => j === ii ? { ...it, text: e.target.value } : it); updSec(si, { items }) }}
                          style={{ flex: 1, border: 'none', background: 'transparent', fontSize: 13, outline: 'none' }} />
                        <button onClick={() => updSec(si, { items: sec.items.filter((_, j) => j !== ii) })}
                          style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#d8d4cc', display: 'flex' }}><X size={12} /></button>
                      </div>
                    ))}
        ...
        <div style={{ background: '#fff', borderRadius: 14, border: '0.5px solid #d8d4cc', padding: 16 }}>
          <p style={{ fontSize: 11, fontFamily: 'monospace', color: '#9c9890', letterSpacing: '0.08em', marginBottom: 10 }}>現場写真を追加</p>
          <div {...getRootProps()}
            style={{ border: `2px dashed ${isDragActive ? '#1a1916' : '#d8d4cc'}`, borderRadius: 12, padding: '24px 16px', textAlign: 'center', cursor: 'pointer', transition: 'all 0.15s' }}>
            <input {...getInputProps()} />
